@echo off
REM ================================================================
REM COMPLETE DEFECT DETECTION PIPELINE (Virtual Environment Version)
...
echo.
echo ================================================================
echo  DEFECT DETECTION PIPELINE - STARTING (VENV VERSION)
echo ================================================================
...
echo  Environment: Virtual Environment with MMDetection
echo ================================================================
echo.
...
python -c "import mmdet; print('MMDetection version:', mmdet.__version__)" >nul 2>&1
if %errorlevel% neq 0 (
    echo ERROR: MMDetection not found in current environment!
...
echo ✓ Virtual environment detected with MMDetection installed
echo.
...
echo   - Using mim for virtual environment compatibility
echo.

mim train mmdet model_configs/sv_model.py
if %errorlevel% neq 0 (
    echo ERROR: SV model training failed!
    echo Trying alternative method...
    python -m mmdet.tools.train model_configs/sv_model.py
    if !errorlevel! neq 0 (
        echo ERROR: Both training methods failed for SV model!
        pause
        exit /b 1
    )
)
...
mim train mmdet model_configs/ev_model.py
if %errorlevel% neq 0 (
    echo ERROR: EV model training failed!
    echo Trying alternative method...
    python -m mmdet.tools.train model_configs/ev_model.py
    if !errorlevel! neq 0 (
        echo ERROR: Both training methods failed for EV model!
        pause
        exit /b 1
    )
)
...
echo   - Using virtual environment compatible paths
echo.
...
REM Export SV model (using virtual environment compatible method)
echo Exporting SV model to sv_model.onnx...
python -m mmdet.tools.deployment.pytorch2onnx ^
...
if %errorlevel% neq 0 (
    echo ERROR: SV ONNX export failed with module method!
    echo Trying direct script method...
    python "%VIRTUAL_ENV%\Lib\site-packages\mmdet\tools\deployment\pytorch2onnx.py" ^
        model_configs/sv_model.py ^
        "!SV_CHECKPOINT!" ^
        --output-file sv_model.onnx ^
        --shape 500 1024 ^
    ...
    if !errorlevel! neq 0 (
        echo ERROR: Both ONNX export methods failed for SV model!
        pause
        exit /b 1
    )
)

REM Export EV model (using virtual environment compatible method)
...
if %errorlevel% neq 0 (
    echo ERROR: EV ONNX export failed with module method!
    echo Trying direct script method...
    python "%VIRTUAL_ENV%\Lib\site-packages\mmdet\tools\deployment\pytorch2onnx.py" ^
        model_configs/ev_model.py ^
        "!EV_CHECKPOINT!" ^
    ...
    if !errorlevel! neq 0 (
        echo ERROR: Both ONNX export methods failed for EV model!
        pause
        exit /b 1
    )
)
...
echo ================================================================
echo  PIPELINE COMPLETED SUCCESSFULLY! (VENV VERSION)
echo ================================================================
echo.
...
echo ✓ Virtual environment compatibility ensured
...
echo   - Used 'mim train mmdet' for training compatibility
echo   - Used 'python -m mmdet.tools.deployment.pytorch2onnx' for export
echo   - Fallback methods included for maximum compatibility
echo.
echo NEXT STEPS:
